...
<div class="container py-4 anim-fade" style="max-width: 1400px;">
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <form action="{{ route('admin.users') }}" method="GET">
                <div class="input-group">
                    <input type="text" name="search" class="form-control input-search-mono" placeholder="Cari nama atau email..." value="{{ request('search') }}">
    ...
    <div class="card card-monochrome">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-center text-muted font-monospace">
                            {{ method_exists($listUsers, 'firstItem') ? $listUsers->firstItem() + $index : $index + 1 }}
                        </td>
                        <td class="fw-medium text-dark">
                            <i class="bi bi-person-circle me-1.5 text-muted"></i> {{ $item->name }}
                        </td>
                        <td class="text-muted font-monospace small">
                            {{ $item->email }}
                        </td>
                        <td class="text-center">
                            @if(strtolower($item->role->name ?? $item->role ?? 'kasir') == 'admin')
                        ...
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-1">
                                <a href="{{ route('admin.users.edit', $item->id) }}" class="btn btn-action" title="Edit Akun">
        ...
        {{-- PAGINATION --}}
        @if(is_object($listUsers) && method_exists($listUsers, 'hasPages') && $listUsers->hasPages())
