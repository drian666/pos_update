    ...
    public function store(Request $request)
{
    ...
    DB::transaction(function () use ($request) {
        ...
        if ($product->stok < $request->quantity) {
            throw \Illuminate\Validation\ValidationException::withMessages(['quantity' => 'Produk stok tidak mencukupi']);
        }
    ...
    public function update(Request $request, ItemPenjualan $itempenjualan)
{
    ...
    DB::transaction(function () use ($request, $itempenjualan) {
        $sale = Penjualan::lockForUpdate()->findOrFail($itempenjualan->penjualan_id);
        $this->authorize('update', $sale);
        $itempenjualan = ItemPenjualan::lockForUpdate()->findOrFail($itempenjualan->id);
        ...
        if ($selisih > 0) {
            if ($produk->stok < $selisih) {
                throw \Illuminate\Validation\ValidationException::withMessages(['quantity' => 'Stok tidak mencukupi']);
            }
    ...
    public function destroy(ItemPenjualan $itempenjualan)
{
    ...
    DB::transaction(function () use ($itempenjualan) {
        $sale = Penjualan::lockForUpdate()->findOrFail($itempenjualan->penjualan_id);
        abort_unless(strtoupper($sale->status) === 'OPEN', 403);
        $itempenjualan = ItemPenjualan::lockForUpdate()->findOrFail($itempenjualan->id);
