    ...
    <div class="container py-4 anim-fade" style="max-width: 1400px;">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center pb-3 mb-4 border-bottom border-secondary border-opacity-10">
            ...
            <div class="mt-3 mt-md-0">
                <a href="<?php echo e(route('penjualan.create')); ?>" class="btn btn-black">
                ...
                <form action="<?php echo e(route('penjualan.index')); ?>" method="GET">
                    <div class="input-group">
                        <input type="text" name="search" class="form-control input-search-mono"
                            placeholder="Cari nota / kasir..." value="<?php echo e(request('search')); ?>">
            ...
            <div class="table-responsive">
                <table class="table table-monochrome align-middle">
                    ...
                    <tbody>
                        ...
                        <?php $__empty_1 = true; $__currentLoopData = $dataPenjualan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr>
                                <td class="text-center text-muted font-monospace">
                                    <?php echo e(method_exists($dataPenjualan, 'firstItem') ? $dataPenjualan->firstItem() + $index : $index + 1); ?>

                                </td>
                                <td class="fw-semibold text-dark font-monospace">
                                    <?php echo e($item->no_nota ?? ($item->invoice ?? 'INV-' . $item->id)); ?>
                                ...
                                <td>
                                    <span class="badge rounded-pill border px-2 py-1"
                                        ...
                                        <?php echo e($item->user->name ?? ($item->kasir ?? 'Admin')); ?>
                                ...
                                <td class="text-end font-monospace">
                                    <?php echo e($item->itemPenjualan ? $item->itemPenjualan->sum('kuantitas') : 0); ?>
                                ...
                                <td class="text-center">
                                    <?php $status = strtolower($item->status ?? 'completed'); ?>
                                ...
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-1">
                                        <a href="<?php echo e(route('penjualan.show', $item->id)); ?>" class="btn btn-action" title="Detail">
                                            <i class="bi bi-eye-fill"></i> Detail
                                        </a>

                                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('update', $item)): ?>
                                        <a href="<?php echo e(route('penjualan.edit', $item->id)); ?>" class="btn btn-action" title="Edit transaksi OPEN"><i class="bi bi-pencil-fill"></i> Edit</a>
                                        <?php endif; ?>
                                        <?php $isCompleted = strtolower($item->status ?? 'completed') === 'completed'; ?>

                                        <?php if(!$isCompleted && auth()->user()->can('delete', $item)): ?>
...
<div class="card-footer bg-white border-top d-flex justify-content-between align-items-center py-3">
    <div class="text-muted small">
        Menampilkan <?php echo e($penjualans->count()); ?> data terbaru
    </div>
</div>
        </div>
    </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\apk_posbaru\resources\views/penjualan/index.blade.php ENDPATH**/ ?>